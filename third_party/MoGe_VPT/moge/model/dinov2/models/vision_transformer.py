...
class DinoVisionTransformer(nn.Module):
    ...
    # >>>>>>>>>>> Modified for VPT >>>>>>>>>>>
    def _get_intermediate_layers_not_chunked(self, x, n=1, use_prompt=False):
    ...
    def get_intermediate_layers(
        ...
        if self.chunked_blocks:
            if use_prompt:
                raise NotImplementedError("Prompt is not supported for chunked blocks")
            outputs = self._get_intermediate_layers_chunked(x, n)
        else:
            outputs = self._get_intermediate_layers_not_chunked(x, n, use_prompt)
        # <<<<<<<<<<< Modified for VPT <<<<<<<<<<<
        if norm:
