...
class DINOv2Encoder(nn.Module):
    ...
    # >>>>>>>>>>> Modified for VPT >>>>>>>>>>>
    def forward(self, image: torch.Tensor, token_rows: Union[int, torch.LongTensor], token_cols: Union[int, torch.LongTensor], return_class_token: bool = False, use_prompt: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        features = self.backbone.get_intermediate_layers(image_14, n=self.intermediate_layers, return_class_token=True, use_prompt=use_prompt)
    # <<<<<<<<<<< Modified for VPT <<<<<<<<<<<
