...
class MoGeModel(nn.Module):
    ...
    # >>>>>>>>>>> Modified for VPT >>>>>>>>>>>
    def forward(self, image: torch.Tensor, num_tokens: int, use_prompt: bool = False, cast_dtype_vit=torch.bfloat16, cast_dtype_head=torch.float32) -> Dict[str, torch.Tensor]:
    # <<<<<<<<<<< Modified for VPT <<<<<<<<<<<
        ...
        # >>>>>>>>>>> Modified for VPT >>>>>>>>>>>
        with torch.autocast(device_type=self.device.type, dtype=cast_dtype_vit):
            # Backbones encoding
            features, cls_token = self.encoder(image, base_h, base_w, return_class_token=True, use_prompt=use_prompt)
        ...
        features = [_feature.float() for _feature in features]

        with torch.autocast(device_type=self.device.type, dtype=cast_dtype_head):
            # Heads decoding
    ...
    def infer(
        ...
        # >>>>>>>>>>> Modified for VPT >>>>>>>>>>>
        points, normal, mask, metric_scale = (output.get(k, None) for k in ['points', 'normal', 'mask_float', 'metric_scale'])
        # <<<<<<<<<<< Modified for VPT <<<<<<<<<<<
    ...
    def infer_naive_prompt(
        ...
        # Forward pass
        output = self.forward(image, num_tokens=num_tokens, use_prompt=use_prompt, cast_dtype_vit=cast_dtype_vit, cast_dtype_head=cast_dtype_head)
        points_raw, mask_float, metric_scale = (output[k] for k in ["points", "mask_float", "metric_scale"])
        mask_binary = mask_float > 0.5

        # Apply mask
        if apply_mask:
            points_raw = torch.where(mask_binary[..., None], points_raw, torch.inf)
