...
class DinoVisionTransformer(nn.Module):
    ...
    # >>>>>>>>>>> Modified for VPT >>>>>>>>>>>
    def init_prompt_tokens(self, token_len: int, init_method: str):
        # TODO: add initialzation method
        ...
        init_method_pref = init_method.split("_")[0]
        if "zeros" == init_method_pref:
            torch.nn.init.zeros_(self.prompt_tokens)
        elif "normal" == init_method_pref:
            init_std = float(init_method.split("_")[1])
            torch.nn.init.normal_(self.prompt_tokens, std=init_std)
        elif "xavier" == init_method_pref:
            torch.nn.init.xavier_normal_(self.prompt_tokens)
        elif "kaiming" == init_method_pref:
    ...
    # >>>>>>>>>>> Modified for VPT >>>>>>>>>>>
    def forward(self, x, masks=None, use_prompt: bool = False):
    # <<<<<<<<<<< Modified for VPT <<<<<<<<<<<
        ...
        # >>>>>>>>>>> Modified for VPT >>>>>>>>>>>
        if use_prompt:
            assert not self.training, "Only use prompt token in eval"
            assert self.prompt_tokens is not None, "prompt_tokens not initialized"
            assert self.frozen_stages <=0, "can not run with prompt tuning at frozen stage"
        ...
        for i, blk in enumerate(self.blocks):
            with (
                ...
                if use_prompt:
                    x = torch.cat([x[:, :1], self.prompt_tokens[0].expand(x.shape[0], -1, -1), x[:, 1:]], dim=1)
                
                # Original forward
                x = blk(x)

                # remove prompt tokens before output
                if use_prompt:
                    x = torch.cat([x[:, :1], x[:, (self.token_len+1):, :]], dim=1)
        # <<<<<<<<<<< Modified for VPT <<<<<<<<<<<
