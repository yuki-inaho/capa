...
class UniDepthV2(
    ...
    def infer(
        ...
        _, model_outputs = self.encode_decode(
            inputs={"image": rgb, "camera": camera}, image_metas=[], use_prompt=use_prompt
        )
        # <<<<<<<<<<< Modified for VPT <<<<<<<<<<<
    ...
    # >>>>>>>>>>> Modified for VPT >>>>>>>>>>>
    def encode_decode(self, inputs, image_metas=[], use_prompt: bool = False):
    # <<<<<<<<<<< Modified for VPT <<<<<<<<<<<
        B, _, H, W = inputs["image"].shape
        ...
        # >>>>>>>>>>> Modified for VPT >>>>>>>>>>>
        features, tokens = self.pixel_encoder(inputs["image"], use_prompt=use_prompt)
        # <<<<<<<<<<< Modified for VPT <<<<<<<<<<<
        inputs["features"] = [
